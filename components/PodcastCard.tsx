...
export default function PodcastCard({
  ...
  return (
    <div
      className={`group relative bg-paper-strong rounded-2xl border-2 border-line ${colorStyle.border} ${colorStyle.hoverBg} hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 card-3d overflow-hidden animate-slide-up flex flex-col`}
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
          <div className="bg-white/95 backdrop-blur-sm text-ink px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 shadow-lg">
            Listen
        ...
        {podcast.featured && (
          <div className="absolute top-3 left-3 z-10">
            <div className="bg-signal-gold/95 backdrop-blur-sm text-ink px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1 shadow-md">
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <a
          ...
          className="block"
        >
          <h2 className="text-xl font-bold text-ink mb-1 group-hover:text-signal-blue transition-colors leading-tight">
            {podcast.name}
          </h2>
        </a>
        ...
        {firebaseEnabled && podcast.id && (
          <div className="mt-4 pt-4 border-t border-line flex items-center justify-between">
            <button
              ...
              aria-label={
                hasUpvoted
                  ? `You recommended ${podcast.name}. ${podcast.upvotes ?? 0} recommendations.`
                  : `Recommend ${podcast.name}. ${podcast.upvotes ?? 0} recommendations.`
              }
