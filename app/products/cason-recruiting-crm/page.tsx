...
export default function CasonRecruitingCrmPage() {
  return (
    <main className="page-shell" id="main-content" tabIndex={-1}>
              ...
              {pipeline.map((col) => (
                <div key={col.stage} className="min-w-0">
                  ...
                  <div className="mt-3 space-y-2">
                    {col.cards.map((c) => (
                      <div key={c.school} className="border border-line bg-paper-strong p-2.5">
                        <div className="text-sm font-semibold text-ink">{c.school}</div>
              ...
              <ul className="divide-y divide-[#eceee8]">
                {questionnaires.map((q) => (
                  <li key={q.school} className="flex items-center justify-between py-2.5">
                    <span className="text-sm font-medium text-ink">{q.school}</span>
                    <span
        ...
        >
          <Frame label="This week">
            <ul className="space-y-3">
              {tasks.map((t) => (
                <li key={t.text} className="flex items-start gap-3 border border-line bg-paper-strong p-3">
                  <span className="mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center border border-[#9fa89d]" aria-hidden="true" />
                  <span className="flex-1 text-sm leading-6 text-ink">{t.text}</span>
                  <span className="whitespace-nowrap text-xs font-semibold text-signal-gold-ink">Due {t.due}</span>
                </li>
              ))}
            </ul>
          </Frame>
      ...
      <section className="border-t border-line bg-wash-blue">
